...
class mainLib {
	...
	public function rateLevel($accountID, $levelID, $stars, $difficulty, $auto, $demon){
		...
		$userID = $this->getUserID($accountID);
		$query = "UPDATE users SET creatorPoints=creatorPoints+:cp WHERE userID=:id";
		$query = $db->prepare($query);
		$query->execute([':cp' => $stars > 0 ? 1 : 0, ':id' => $userID]);
	...
	public function featureLevel($accountID, $levelID, $state) {
		...
		$query = "UPDATE users SET creatorPoints=creatorPoints+:cp WHERE userID=:id";
		$query = $db->prepare($query);
		$query->execute([':cp' => $state > 0 ? 1 : 0, ':id' => $userID]);
